...
void Config::load() {
    ...
    docker_user_file_ = getEnv("DOCKER_POSTGRES_USER_FILE", "/run/secrets/postgres-u");
    docker_password_file_ = getEnv("DOCKER_POSTGRES_PASSWORD_FILE", "/run/secrets/postgres-p");
}
