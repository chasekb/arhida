...
OaiClient::OaiClient(const std::string& base_url) 
    : base_url_(base_url), curl_(nullptr), rate_limit_delay_(3), max_retries_(3) {
...
void OaiClient::setMaxRetries(int max_retries) {
    max_retries_ = max_retries;
}

size_t OaiClient::writeCallback(void* contents, size_t size, size_t nmemb, void* userp) {
...
std::vector<Record> OaiClient::listRecords(
    ...
    while (retries < max_retries_) {
        try {
            ...
            retries++;
            spdlog::warn("Request failed (attempt {}/{}): {}", retries, max_retries_, e.what());
        ...
        if (xmlStrcmp(node->name, (const xmlChar*)"record") == 0) {
            ...
            for (xmlNodePtr child = node->children; child; child = child->next) {
                if (xmlStrcmp(child->name, (const xmlChar*)"header") == 0) {
                    for (xmlNodePtr header_child = child->children; header_child; header_child = header_child->next) {
                        if (xmlStrcmp(header_child->name, (const xmlChar*)"identifier") == 0) {
                            record.header_identifier = (const char*)xmlNodeGetContent(header_child);
                        } else if (xmlStrcmp(header_child->name, (const xmlChar*)"datestamp") == 0) {
                            record.header_datestamp = (const char*)xmlNodeGetContent(header_child);
                        } else if (xmlStrcmp(header_child->name, (const xmlChar*)"setSpec") == 0) {
                            record.header_setSpecs.push_back((const char*)xmlNodeGetContent(header_child));
                        }
                    }
                }
                // Parse metadata (Dublin Core)
                    ...
                    for (xmlNodePtr dc = child->children; dc; dc = dc->next) {
                        ...
                        for (xmlNodePtr dc_child = dc->children; dc_child; dc_child = dc_child->next) {
