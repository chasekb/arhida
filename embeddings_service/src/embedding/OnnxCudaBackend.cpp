...
#if ARHIDA_HAS_ORT
  try {
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "arhida-embeddings-cuda");
...
OnnxCudaBackend::OnnxCudaBackend(EmbeddingServiceConfig config)
...
void OnnxCudaBackend::initialize() {
  ...
  if (!tokenizer_initialized_) {
    throw std::runtime_error(
        "OnnxCudaBackend failed tokenizer initialization (empty tokenizer vocabulary)");
  }

  session_initialized_ = initializeOnnxSession(config_);
  if (!session_initialized_) {
    throw std::runtime_error(
        "OnnxCudaBackend failed ONNX session initialization for model: " +
        config_.model_path);
  }
...
OnnxCudaBackend::embed(const BatchInput& input) {
  ...
  for (const auto& text : input) {
    ...
    vectors.push_back(makeNormalizedVectorFromTokens(
        token_ids, config_.model_dimension, 0xC0FFEE5678ULL));
  }
  return vectors;
}
